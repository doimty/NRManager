...
The staged launchd plist is verified per lane, not merely parsed. Roothide must hold `@JBROOT@` plus the relative program and baseline paths, with the placeholder present as plain bytes; rootless must hold `/var/jb` and must not contain `@JBROOT@` anywhere. The container format is recorded as evidence but not constrained, because Theos converts every staged plist to `binary1` in its `internal-package` step, which runs after `before-package`: an XML requirement here failed a package that was completely correct. This gate exists because the repository template already carries the placeholder: a `before-package` step that silently does not run leaves the roothide package accidentally correct and the rootless package holding a literal `@JBROOT@` program path that nothing on that lane ever substitutes, so the daemon could never start. Every per-stage check passes in that state, which is why the end-to-end test carries an explicit negative control that stages with the patch step skipped and requires both lanes to fail. That test also reproduces the Theos binarization step, so a format assumption cannot pass locally and fail in the cloud again. Because the two `grep` flavours disagree about binary files, the shell-script suite additionally reruns itself in full against a BSD-flavoured `grep`; a device whose `grep` stops at the first NUL must reach the same outcome as one whose `grep` scans the whole buffer.
