...
- **Whether a substitution is needed is decided by the lane, not by reading the plist, and the format is settled before `sed` runs.** `plutil` converts to XML first, and the rewrite is refused if that conversion did not happen: where `grep` can see into a binary plist, `sed` would expand 8 bytes to a longer path inside a length-prefixed string and leave the trailer's offset table stale. launchd could then not parse the file at all, while every later check passed because the placeholder really was gone. A plist left unsubstituted is recoverable by reinstalling; a corrupted one reported as written is not. What must not decide any of this is the file's contents. Theos converts every staged plist to `binary1` in its `internal-package` step, which runs *after* `before-package`, so both lanes ship a binary plist regardless of what the packaging script wrote — and the two `grep` flavours disagree about binary files. GNU `grep -a` finds `@JBROOT@` inside a binary plist; the BSD `grep` on macOS reports no match for the same file and pattern, and *no match is the success branch*. Asking the file would therefore let the roothide placeholder survive with nothing in the install log on a bootstrap whose `grep` behaves that way, which is precisely the silent-success failure this script exists to prevent. `NEEDS_JBROOT` is empty for exactly the lanes whose prefix is already baked in, so the lane answers the question without reading anything. For the same reason the package verifier records the plist format as evidence rather than requiring XML; what it requires is that `@JBROOT@` be present in the roothide package as plain bytes.
