...
- **The plist must be confirmed XML before `sed` touches it.** `grep -a` matches `@JBROOT@` inside a binary plist too, so a failed or missing `plutil` would let a textual rewrite expand 8 bytes to a longer path inside a length-prefixed string and leave the trailer's offset table stale. launchd could then not parse the file at all, while every later check passed because the placeholder really was gone. A plist left unsubstituted is recoverable by reinstalling; a corrupted one reported as written is not, so this refuses instead.
