...
- `CCNMAutomaticMaintenanceDecision` is a pure C decision layer compiled into the Settings bundle and the read-only maintenance daemon. It requires two matching clean samples, permits one correction decision, and blocks on unsafe, incompatible, busy, cooldown, verification-pending, or exhausted states.
