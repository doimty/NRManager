...
- `CCNMAutomaticMaintenanceDecision` is a pure C decision layer compiled into the Control Center, Settings bundle, and read-only maintenance daemon. It requires two matching clean samples, records one correction candidate, and blocks on unsafe, incompatible, busy, cooldown, verification-pending, or exhausted states. Repeated evidence for the same candidate returns `DropRecorded`; it does not create another generation.
...
- `CorrectOnce` is still not connected to a setter call. The daemon only persists evidence and evaluates the pure decision layer. Maintenance-record schema 2 therefore keeps `verificationPending` false for a read-only candidate and records it as `DropRecorded`; schema-1 records are rejected because that prototype incorrectly called an observation "verification pending" before any setter existed.
