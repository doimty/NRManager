...
- A policy-scoped root launchd job owns automatic observation. Its single `KeepAlive/PathState` condition uses the same jailbroken-root baseline path as the policy reader. `postinst` registers it after safe guard cleanup; `prerm` stops it before restore; baseline retirement makes the already-running daemon exit.
