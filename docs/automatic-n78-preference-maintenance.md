...
A transaction baseline is not a portable backup. New baselines retain device model, system version/build, complete supported-band evidence, and `modifiedBandKeys`. Legacy baselines remain readable only through the existing accepted-target gate. A portable migration record may carry policy intent and profile identity, but never raw modem BandInfo.
...
- A policy-scoped root launchd job owns automatic observation. Its single `KeepAlive/PathState` condition uses the same jailbroken-root baseline path as the policy reader. The shell `postinst` writes the live jailbreak root into the plist and the install guard registers the job after safe guard cleanup; `prerm` stops it before restore; baseline retirement makes the already-running daemon exit. Registration failure warns rather than failing the install, because the plist on disk is what makes the job loadable at the next boot.
