...
Can the same setter temporarily constrain only the NR allowed-band array to n78 while preserving the complete LTE list needed for NSA anchoring and fallback?

## Stage 3 hypothesis

On the locked target, replacing only `kCTRegistrationRadioAccessTechnologyNR` with exactly `[78]` will eventually read back as requested. If it does, a second read after a monotonic 60-second observation interval will still equal the complete request, after which the original six-technology snapshot will be restored and polled to exact equality.

A read-back that remains equal to the original snapshot is the independent signal that CommCenter ignored the changed request. A read-back matching neither request nor snapshot, any observation-end drift, or any unverified restore is a failure.

## Stage 3 success and failure signals

- The live `activeBands` and `supportedBands` dictionaries must both contain n78 before a payload can be built. If either does not, no recovery files and no setter call are made.
- The requested dictionary has the same six RAT keys as the snapshot. Its NR array is exactly `[78]`; every non-NR array, including the full ordered LTE list, is structurally identical to the snapshot. If the live NR array is already `[78]`, the changed-value probe is refused.
- The final pre-write read must reproduce both complete active and supported dictionaries. `CTBandInfo.activeBands` must then reproduce the validated request exactly before the setter is armed.
- Changed-write visibility is polled once per second for up to two minutes. The 60-second observation starts only after the exact request has actually been observed. At the end of the interval, the complete live dictionary is read again and must still equal the request.
- Whether the request took effect, was ignored, or produced an indeterminate read-back, a normally returned setter is followed by an authorized automatic restore of the complete original dictionary and a two-minute exact read-back poll. Any uncertain setter outcome suppresses same-boot recovery and requires reboot before manual restore.
- Success means a determinate changed-write result, the required observation when n78-only took effect, and verified exact recovery. An ignored changed write can complete without the 60-second observation because n78-only was never observed.
- The Control Center RAT setter takes the same cross-process recovery lock and refuses to write whenever any Band snapshot, intent, setter marker, or restore marker exists. It therefore cannot alter radio mode during the n78 observation or recovery window.

## Guardrails

- No user-selected Band numbers exist in this build. Changed writes are limited to either removing one hard-coded cold LTE band or replacing only the live NR array with the hard-coded `[78]` request.
...
- The Control Center RAT path is fenced by the same recovery lock and durable-state gate. A blocked toggle does not mutate its selected-network preference.
...
- The recovery snapshot, the write-intent record, the setter-in-flight marker, and the restore-in-flight marker are created with `O_EXCL`, durably synced, read back, and never overwritten. A second experiment is refused while any of them exists. The intent repeats the snapshot UUID, creation time, and complete active-band dictionary. Removal intents additionally bind the complete supported-band dictionary, removed band, and full requested dictionary. n78-only intents bind the complete supported-band dictionary, NR key, band 78, 60-second interval, and full requested dictionary. Both changed requests are regenerated and revalidated from the snapshot before recovery is authorized.
...
- `CTBandInfo.activeBands` must exactly equal the intended dictionary before any setter runs: the deep-copied original for same-value and restore writes, the validated single-removal dictionary for the removal write, or the validated n78-only dictionary for the NR write.
...
- Unlike stage 1, the stage 2 and stage 3 requests differ from the prior value. A crash before restore can leave one cold LTE band disabled or leave NR constrained to n78 until the post-reboot manual restore runs. The n78-only request preserves the full LTE array, but it does not force NSA, SA, registration, or service; LTE fallback or a temporary loss of cellular service remains possible.
