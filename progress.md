...
- First CC package upgrade was safely blocked before unpack: both old and fallback new `prerm` received `Permission denied` from the CoreTelephony live probe in dpkg context and exited 73. No product or modem state changed. The corrected `prerm` trusts only the exact completed known-device restore proof (fixed source/digest, clean verified system default, exact historical full six-RAT read-back, no removal guard) before that probe; all other states remain fail-closed. Current installed `prerm` still requires a separately approved, SHA-bound atomic hotfix before the corrected full package can install.
- Full host suite 93/93; `py_compile`, source verifier, and `git diff --check` pass; local rootless compile/package smoke passed. Branch `fix/cc-serving-band-label`; corrected cloud build, prerm hotfix package, and device acceptance still required.
